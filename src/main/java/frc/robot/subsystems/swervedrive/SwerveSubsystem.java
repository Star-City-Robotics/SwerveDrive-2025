...
public class SwerveSubsystem extends SubsystemBase implements DriveTemplate
{
  /**
   * Swerve drive object.
   */
  ...
  double driveConversionFactor = SwerveMath.calculateMetersPerRotation(Units.inchesToMeters(4), 5.35714285715);
    ...
    ChassisSpeeds goalSpeeds;
      if (fieldCentric) {
        ...
        Rotation2d robotRotation =
            isFlipped
                ? getHeading().plus(new Rotation2d(Math.PI)) // Flip orientation for Red Alliance
                : getHeading();
  
        goalSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(speeds, robotRotation);
        goalSpeeds.omegaRadiansPerSecond *= -1.0;
      } else {
        // Logger.recordOutput("Drive/DesiredRobotCentricSpeeds", speeds);
        goalSpeeds = speeds;
      }
      this.swerveDrive.drive(goalSpeeds);
  }
  /**
